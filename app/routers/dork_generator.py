...
async def _llm_generate_dorks(goal: str, target: str | None) -> dict | None:
    ...
    try:
        ...
        if not isinstance(parsed, dict):
            log.warning("Dork LLM returned non-dict JSON")
            return None

        _DORK_ALLOWED_KEYS = {"query", "explanation", "defensive_use", "risk_level"}
        _VALID_RISK = {"info", "sensitive", "high_impact"}

        raw_dorks = parsed.get("dorks") if isinstance(parsed.get("dorks"), list) else []
        sanitized_dorks = []
        for d in raw_dorks:
            if not isinstance(d, dict):
                continue
            risk = safe_str(d.get("risk_level"), 20, "info")
            sanitized_dorks.append({
                "query": safe_str(d.get("query"), 500, ""),
                "explanation": safe_str(d.get("explanation"), 500, ""),
                "defensive_use": safe_str(d.get("defensive_use"), 500, ""),
                "risk_level": risk if risk in _VALID_RISK else "info",
            })
        parsed["dorks"] = sanitized_dorks
        parsed["refused"] = bool(parsed.get("refused"))
        parsed["refusal_reason"] = safe_str(parsed.get("refusal_reason"), 500, None)
        parsed["notes"] = safe_str(parsed.get("notes"), 500, None)
        parsed["_llm_source_note"] = "Dorks generated by Claude Haiku 4.5. Review before use."
